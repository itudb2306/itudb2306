...
from models.tables.batting.records import Records
from models.tables.batting.forms import FilterForm , SortForm
from config import RECORDS_PER_PAGE
import urllib.parse

table_batting_blueprint = Blueprint('batting', __name__)
...
@table_batting_blueprint.route('/batting', methods = ['GET', 'POST'])
def view_table():
    """
    URL: /tables/batting?p=
    """
    # Pagination
    ...
    query = Query().SELECT('p.nameFirst AS FirstName, p.nameLast AS LastName, t.name AS TeamName, b.yearID, b.stint, l.league AS LeagueName, b.G, b.AB, b.R, b.H, b.HR, b.RBI, b.ID'
    ).FROM('batting b ' 
    ).JOIN('people p', 'b.playerID = p.playerID'
    ).JOIN('teamnames t', 'b.team_id = t.id'
    ).JOIN('leagues l', 'b.lgID = l.lgID'
    ).WHERE(filter_string
    ...
    total_pages_query = Query().SELECT('COUNT(*)').FROM('batting b ' 
    ).JOIN('people p', 'b.playerID = p.playerID'
    ).JOIN('teamnames t', 'b.team_id = t.id'
    ).JOIN('leagues l', 'b.lgID = l.lgID'
    ).WHERE(filter_string
    ...
    return render_template('table_batting/table_batting.html', data_list=data.records, current_page=page, total_pages=total_pages, leagues_list = leagues_list, teams_list = teams_list, players_list = players_list, filter = filter_encoded, sort = sort_encoded)


@table_batting_blueprint.route('/batting/details/<string:ID>', methods=['GET', 'POST'])
def view_details(ID):
    """
    URL: /tables/batting/details/<string:ID>
    """
    # Query building for table
    query = Query().SELECT('p.nameFirst AS FirstName, p.nameLast AS LastName, t.name AS TeamName, b.yearID, b.stint, l.league AS LeagueName, b.G, b.AB, b.R, b.H, b.HR, b.RBI, b.ID'
    ).FROM('batting b ' 
    ...
    result = result[0]
    result = dict(zip(['FirstName', 'LastName', 'TeamName', 'yearID', 'stint', 'LeagueName', 'G', 'AB', 'R', 'H', 'HR', 'RBI','ID'], result))


    return render_template('table_batting/batting_details.html', data = result)




@table_batting_blueprint.route('/batting/delete/<string:ID>', methods=['GET', 'POST'])
def delete_record(ID=None):
    """
    URL: /tables/batting/delete/<string:ID>
    """

    if request.method == 'POST' and ID is not None:
        try:
            # Query building for table
            query = Query().DELETE().FROM('batting').WHERE('ID = %s' % ID)
    ...
    return redirect(url_for('batting.view_table'))

@table_batting_blueprint.route('/batting/update/<string:ID>', methods=['GET', 'POST'])
def update_record(ID=None):
    """
    URL: /tables/batting/update/<string:ID>
    """
    ...
    if request.method == 'POST' and ID is not None:
        try:
            ...
            query = Query().UPDATE('batting').SET(col_val_pairs).WHERE('ID = %s' % ID)
            queryString = query.BUILD()
            print(query._PARAMS)
            print(queryString)
...
@table_batting_blueprint.route('/batting/add', methods=['GET', 'POST'])
def add_record():
    """
    URL: /tables/batting/insert
    """
    if request.method == 'POST':
        ...
        # Query building for table
        query = Query().INSERT_INTO('batting').VALUES(col_val_pairs)
        queryString = query.BUILD()
        print(query._PARAMS)
        print(queryString)
        db.execute(queryString, tuple(query._PARAMS))
